...
| `results/voces_v5_results.json` | the run that produced the figures (Qwen2.5-3B, fp16, seed 0) |
| `results/voces_v5_breakage.txt` | the auto-filled breakage / surprise log |
| `results/fig1`, `fig2` | the two figures above, regenerated from the JSON |
...
| `src/make_figures.py` | regenerates both figures from `results/voces_v5_results.json` (CPU, no model) |
...
python src/make_figures.py        # regenerates both figures from results/voces_v5_results.json
```
...
@techreport{voces2026script,
  author = {Pavan, Tomás and {Claude (Opus 4.8)}},
