...
- **Native-tradition recognition control (Talkie-1930) — DONE 2026-06-26.** A FLOP-matched, architecture-identical 13B **base** pair differing only in training era; the recognition pipeline ported to Talkie's custom non-HuggingFace architecture (`src/coercion/build_talkie_probe.py` + the recognition harness). **Recognition-without-representation is era-invariant:** the pre-1931 model recognizes the voces texture (H1 **0.914** Latin / **0.927** Greek, beats surprisal +0.41/+0.40) but the deep decider is null (Greek **p=0.83**, Latin **p=0.44**) — *even a model from the tradition's own era only sees the script, not the spell.* Talkie's English-only vocab fragments Greek hardest of all (**13.0** tok/vox, the study's most extreme). Result JSON: [`results/cross-family/talkie_recognition_1930_results.json`](results/cross-family/talkie_recognition_1930_results.json). The matched modern twin (`talkie-web`) is a **pending** confirming control (harness ran; analysis not captured; near-certain null). Contrast: on the *coercion* axis the same era-pair shows an **effect** ("native = distinct", sequel §4c) — two era-pair controls, two opposite shapes. See `results/cross-family/FINDINGS.md` §5.
